...
fn main() {
    ...
    println!("cargo:rerun-if-changed=src/shaders"); // Ideally we could watch individual files

    let mut compiler = Wesl::new("src/shaders");
    compiler.set_custom_mangler(NoMangler); // Hack to get wgsl_to_wgpu to generate nice types

    shaders.push(compile_shader(
        &compiler,
        "compute_patches.wesl",
        "compute_patches",
    ));
    shaders.push(compile_shader(
        &compiler,
...
fn compile_shader(compiler: &Wesl<wesl::StandardResolver>, root: &str, name: &str) -> ShaderModule {
    let generated = match compiler.compile(root) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("{e}");
            panic!("WESL error");
