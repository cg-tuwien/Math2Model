...
pub struct GpuApplication {
    ...
    indirect_draw_buffers: Vec<TypedBuffer<copy_patches::DrawIndexedIndirectArgs>>,
    meshes: Vec<Mesh>,
...
impl GpuApplication {
    pub async fn new(
        ...
        let indirect_draw_buffers = meshes
            .iter()
            .zip(patch_sizes())
            .map(|(mesh, patch_size)| {
                TypedBuffer::new_storage(
                    &device,
                    &format!("Indirect Draw Buffer {}", patch_size),
        ...
        let copy_patches_bind_group_0 = copy_patches::bind_groups::BindGroup0::from_bindings(
            ...
            copy_patches::bind_groups::BindGroupLayout0 {
                ...
                render_buffer_16: render_buffer[3].as_entire_buffer_binding(),
                render_buffer_32: render_buffer[4].as_entire_buffer_binding(),
                indirect_draw_2: indirect_draw_buffers[0].as_entire_buffer_binding(),
                indirect_draw_4: indirect_draw_buffers[1].as_entire_buffer_binding(),
                indirect_draw_8: indirect_draw_buffers[2].as_entire_buffer_binding(),
                indirect_draw_16: indirect_draw_buffers[3].as_entire_buffer_binding(),
                indirect_draw_32: indirect_draw_buffers[4].as_entire_buffer_binding(),
            },
        );
    ...
    pub fn render(&mut self, render_data: RenderData) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            for ((bind_group_1, mesh), indirect_draw_buffer) in self
                .render_bind_group_1
                .iter()
                .zip(self.meshes.iter())
                .zip(self.indirect_draw_buffers.iter())
            {
                shader::set_bind_groups(&mut render_pass, &self.render_bind_group_0, &bind_group_1);
                render_pass.set_vertex_buffer(0, mesh.vertex_buffer.slice(..));
                render_pass
                    .set_index_buffer(mesh.index_buffer.slice(..), wgpu::IndexFormat::Uint16);
                render_pass.draw_indexed_indirect(indirect_draw_buffer.buffer(), 0)
            }
        }
