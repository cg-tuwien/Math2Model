...
export class ExporterInstance {
    ...
    const vertCount = vertsMapping
        ? vertsMapping.size
        : this.patches.reduce((acc: number, patch: any[]) => acc + patch.length, 0);
...
private processPatch(patch: any[], step: number): number {
    ...
    const v0 = patch[0], v1 = patch[1], v2 = patch[2], v3 = patch[3];
    ...
    for (let i = 0; i < verticesPatch.length - 1; i++) {
        if (verticesPatch[i].uv.y === verticesPatch[i + 1].uv.y &&
