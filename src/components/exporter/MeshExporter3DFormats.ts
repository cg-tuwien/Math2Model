...
export class MeshExporter3DFormats {
    ...
    public async exportModel(format: string): Promise<any> {
        if (format == "obj") {
            ...
            return { data: file[0], binary: false, extraFile: {data:file[1],fileExtension:"mtl"} };
        }
        if (format == "glb") {
    ...
    public objExport(meshes: any[]): [string, string] {
        ...
        for (let i = 0; i < meshes.length; i++) {
            ...
            for (let vert of buf.verts) {
                ...
                mexpstring += "v " + vecResult[0] + " " + vecResult[1] + " " + vecResult[2] + "\n";
            }
            if (this.useUvs) {
    ...
    public async gltfExport(meshes: any[], binary: boolean): Promise<Uint8Array> {
        ...
        for (let x = 0; x < meshes.length; x++) {
            ...
            console.log(inMaterial);
            let material = doc.createMaterial(materialName)
                .setBaseColorFactor([inMaterial.color [0], inMaterial.color[1], inMaterial.color[2], 1.])
            ...
            node.setTranslation(
                ...
                [meshBuf.scale, meshBuf.scale, meshBuf.scale]
            ).setRotation(
                eulerToQuaternion(meshBuf.rotation)
            );
            scene.addChild(node);
        }
